...
PARAMS = {
    ...
    "rock_z_score_threshold": 2,
    "rock_z_score_take_profit": 0.01
}
...
class Trader:
        ...
        for product in products:
            ...
            if product == Product.CROISSANTS:
                ...
                if Product.PB2 in state.order_depths:
                    od_pb2 = deepcopy(state.order_depths[Product.PB2])
                    pos_pb2 = state.position.get(Product.PB2, 0)

                    if buy_time_diff < 1000:
                        price_pb2 = last_buy_price_pb2 * 1.5
                        pb2_orders += self.go_fully_long(Product.PB2, od_pb2, price_pb2, pos_pb2, self.LIMIT[Product.PB2])
                    elif sell_time_diff < 1000:
                        price_pb2 = last_sell_price_pb2 * 0.5
                        pb2_orders += self.go_fully_short(Product.PB2, od_pb2, price_pb2, pos_pb2, self.LIMIT[Product.PB2])
                    
                    result[Product.PB2] = pb2_orders
    ...
    def mean_reversion_orders(
        ...
        orders = []
        mid_price = self.swmid(order_depth)
        ...
        # z_score_take_profit = 0.01


        # z_score_threshold = self.params["rock_z_score_threshold"]
        # z_score_take_profit = self.params["rock_z_score_take_profit"]
        z_score_threshold = self.params["rock_z_score_threshold"]
        z_score_take_profit = self.params["rock_z_score_take_profit"]
        # z_score_threshold = 1.5
        # z_score_take_profit = 0

        # z_score_threshold = 3.7
        # z_score_take_profit = 0.1
    ...
    def run(self, state: TradingState):
        ...
        if Product.VOLC_ROCK in state.order_depths:
            vr_position = state.position.get(Product.VOLC_ROCK, 0)
            vr_order_depth = state.order_depths[Product.VOLC_ROCK]
            ...
            result[Product.VOLC_ROCK] = vr_orders
        ...
        #         od = deepcopy(state.order_depths[Product.PB2])
        #         if od:
        #             best_bid = max(od.buy_orders.keys())
        #             best_ask = min(od.sell_orders.keys())
        #             mid = (best_bid + best_ask) / 2
        ...
        #         traderObject["last_olivia_ink_buy_price"] = t.price
        #     elif t.seller == "Olivia":
        #         traderObject["last_olivia_ink_sell_trade"] = t.timestamp
        #         traderObject["last_olivia_ink_sell_price"] = t.price
