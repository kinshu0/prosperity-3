...
PARAMS = {
    ...
    Product.PB1: {"spread_threshold": 50, "clear_thresh": 5, "max_qty_per_leg": 25},
...
class Trader:
    ...
    def merle_trade_vouchers(self: "Trader", state: TradingState,
        ...
        for K, symbol in {
            ...
            # TODO why are we continuing here?
    ...
    def trade_baskets(self: "Trader", state: TradingState, result: dict[str,list[Order]], mem: dict):
        ...
        for basket, recipe in BASKET_RECIPE.items():
            ...
            clear_thresh = self.params[basket]['clear_thresh']
            ...
            # if abs(diff) < clear_thresh:
            #     # clear position
            #     print("AM CLEANING")
            #     if pos_b > 0:
            #         result[basket].append(Order(basket, ask_b, -pos_b))
            #     elif pos_b < 0:
            #         result[basket].append(Order(basket, bid_b, -pos_b))
    ...
    def macaron_arb(self, state: TradingState, result: dict[str,list[Order]]):
        ...
        # buy_orders = state.order_depths[Product.MAC].buy_orders
        # sell_orders = state.order_depths[Product.MAC].sell_orders
        # mainland_bid, best_bid_vol = sorted(buy_orders.items(), reverse=True)[0]
        # mainland_ask, best_ask_vol = sorted(sell_orders.items(), reverse=False)[0]

        # # edge from long on mainland and conversion by short to island

        # island_bid = obs.bidPrice
        # island_ask = obs.askPrice

        # long_cost = obs.exportTariff + obs.transportFees + 1
        # short_cost = obs.importTariff + obs.transportFees

        # long_edge = mainland_ask - island_bid
        # short_edge = island_ask - mainland_bid

        # orders = []

        # if long_edge > long_cost and long_edge > 1:
        #     orders = [Order(Product.MAC, mainland_ask, 10)]

        # elif short_edge > short_cost and short_edge > 1:
        #     orders = [Order(Product.MAC, mainland_bid, -10)]
    ...
    def goated_person_trades(self, state: TradingState, result: dict, trader_data: dict):
        products = [Product.CROISSANTS, Product.INK]
    ...
    def run(self, state: TradingState):
        ...
        # ## --------------------- ROUND 2 BASKETS, DJEMBES, JAMS, & CROISSANTS --------------------- ###
        # self.trade_baskets(state, result, traderObject)
        
        # # # VOUCHER ARBITRAGE
        # # --------------------- ROUND 3 ROCKS AND VOUCHERS --------------------- ###
        # self.merle_trade_vouchers(state, result, traderObject)


        ### --------------------- ROUND 4 MACROOOOOOOONS --------------------- ###
        # if Product.MAC in state.order_depths:
        #     # conversions = self.juan_official_get_macarons_orders(state, result)
        #     conversions = self.macaron_arb(state, result)


        
        # ### --------------------- ROUND 5 CODE WITH TRADERS --------------------- ###
        # # store the last timestamp of Olivia's trades
        # trades = state.market_trades.get(Product.CROISSANTS, [])
        # for t in trades:
        #     if t.buyer == "Olivia":
        #         traderObject["last_olivia_croissant_buy_trade"] = t.timestamp
        #         traderObject["last_olivia_croissant_buy_trade_price"] = t.price
        #     elif t.seller == "Olivia":
        #         traderObject["last_olivia_croissant_sell_trade"] = t.timestamp
        #         traderObject["last_olivia_croissant_sell_trade_price"] = t.price
        # trades = state.market_trades.get(Product.INK, [])
        # for t in trades:
        #     if t.buyer == "Olivia":
        #         traderObject["last_olivia_ink_buy_trade"] = t.timestamp
        #         traderObject["last_olivia_ink_buy_trade_price"] = t.price
        #     elif t.seller == "Olivia":
        #         traderObject["last_olivia_ink_sell_trade"] = t.timestamp
        #         traderObject["last_olivia_ink_sell_trade_price"] = t.price
        
        # self.goated_person_trades(state, result, traderObject)
