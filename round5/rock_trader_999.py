...
class Trader:
    ...
    def fair_rock(self, order_depths: dict[str, OrderDepth], trader_data: dict):
        beta = 0.95
        ink_prices = self.get_price_history(Product.VOLC_ROCK, trader_data)
        mid = self.swmid(order_depths[Product.VOLC_ROCK])
        max_prices_len = 21
        if len(ink_prices) < max_prices_len:
            return mid
        pct_ink_past = ink_prices[-1] / ink_prices[-max_prices_len] - 1
        pct_ink_future = pct_ink_past * beta
        fair_ink = mid * (1 + pct_ink_future)
        self.update_price_history(mid, Product.VOLC_ROCK, max_prices_len, trader_data)
        return fair_ink
    
    def rock(self, order_depths: dict[str, OrderDepth], trader_data: dict, position: int):
        beta = 0.95
        ink_prices = self.get_price_history(Product.VOLC_ROCK, trader_data)
        mid = self.swmid(order_depths[Product.VOLC_ROCK])
        max_prices_len = 21
        ...
        # ink_fair = self.fair_ink(order_depths, trader_data)
        # ink_mid = self.swmid(order_depths[Product.VOLC_ROCK])
        ink_orders = []
        # diff = ink_mid - ink_fair
        quantity = 1
        # # mean_diff = 76
        ...
        # if diff > thresh:
        #     print('SELLING INK')
        #     ink_orders.append(Order(Product.VOLC_ROCK, 0, -1*quantity))
        # elif diff < -thresh:
        #     print('BUYING INK')
        #     ink_orders.append(Order(Product.VOLC_ROCK, 999999, 1*quantity))
    ...
    def run(self, state: TradingState) -> dict[str, list[Order]]:
        ...
        if Product.VOLC_ROCK in state.order_depths:
            order_depths = deepcopy(state.order_depths)
            rock_position = state.position.get(Product.VOLC_ROCK, 0)
            rock_orders = self.rock(order_depths, trader_data, rock_position)
            # print(ink_orders)
            result[Product.VOLC_ROCK] = rock_orders
