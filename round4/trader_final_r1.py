...
PARAMS = {
    ...
    Product.KELP: {
        ...
        "adverse_volume": 15,
        "reversion_beta": 2.2756650754884564,
        "disregard_edge": 1,
        "join_edge": 0,
        "default_edge": 1,
    },
...
class Trader:
    def __init__(self, params=None):
        ...
        self.LIMIT = {Product.RESIN: 50, Product.KELP: 75, Product.INK: 50}
    ...
    def kelp_fair_value(self, order_depth: OrderDepth, traderObject, sugar_price) -> float:
    ...
    def run(self, state: TradingState):
        ...
        observation = state.observations.conversionObservations.get(Product.KELP)
        ...
        if Product.KELP in self.params and Product.KELP in state.order_depths:
            ...
            kelp_fair_value = self.kelp_fair_value(
                state.order_depths[Product.KELP], traderObject, sugar_price
            )
            if sunlightIndex > 45:
                od = deepcopy(state.order_depths[Product.KELP])
                print('sunlightIndex', sunlightIndex)
            else:
                od = OrderDepth()
                od.buy_orders = {}
                od.sell_orders = {}
