...
class Trader:
    ...
    def ink(self, order_depth: OrderDepth, position: int, trader_data: dict) -> list[Order]:
        ...
        if ink_trigger_hist[-101:] == 1:
            if position > 0:
                orders = self.limit_sell(Product.INK, od, best_market_bid, position, 0)
            elif position < 0:
                orders = self.limit_buy(Product.INK, od, best_market_ask, position, 0)
        
        if ink_trigger_hist[-102:] == 1:
            if position > 0:
                orders = self.limit_sell(Product.INK, od, best_market_bid, position, 0)
            elif position < 0:
                orders = self.limit_buy(Product.INK, od, best_market_ask, position, 0)
...
#         position_hist = trader_data.get('ink_position_hist', [])
...
#             # neutralize position
#             if position > 0:
#                 orders = self.limit_sell(Product.INK, od, mean_price - 2, position, 0)
#                 # orders = self.limit_sell(Product.INK, od, best_market_bid, position, 0)
#             elif position < 0:
#                 orders = self.limit_buy(Product.INK, od, mean_price + 2, position, 0)
#                 # orders = self.limit_buy(Product.INK, od, best_market_ask, position, 0)
