...
PARAMS = {
    'ink_change_threshold_pct': 0.03,
    'ink_window_size': 500,
    'ink_position_limit': 50,
    'ignore_window_size': 50,
    'price_percent_needed': 0.02,
...
class Trader:
    ...
    def limit_buy(self, product: str, order_depth: OrderDepth, limit_price: int | float, position: int, position_limit: int, trader_data) -> tuple[list[Order], int, int]:
        to_buy = position_limit - position
        ...
        for price, volume in market_sell_orders:
            if to_buy > 0 and price <= max_buy_price:
                quantity = min(to_buy, -volume)
                own_orders.append(Order(product, price, quantity))
                print("BUY", product, price, quantity)
    ...
    def limit_sell(self, product: str, order_depth: OrderDepth, limit_price: int | float, position: int, position_limit: int, trader_data) -> tuple[list[Order], int, int]:
        to_sell = position - -position_limit
        ...
        for price, volume in market_buy_orders:
            if to_sell > 0 and price >= min_sell_price:
                quantity = min(to_sell, volume)
                own_orders.append(Order(product, price, -quantity))
                print("SELL", product, price, -quantity)
    ...
    def ink(self, order_depth: OrderDepth, position: int, trader_data: dict) -> list[Order]:
        ...
        ignore_window_size = self.params['ignore_window_size']

        # change_threshold_pct = 0.015
        ...
        # calculate the mean price without the last 5 prices
        if len(ink_price_hist) > ignore_window_size:
            mean_price = sum(ink_price_hist[:-ignore_window_size]) / (len(ink_price_hist) - ignore_window_size)
        ...
        if delta_pct >= change_threshold_pct:
            # sell
            orders = self.limit_sell(Product.INK, od, best_market_bid, position, position_limit, trader_data)
            threshold_triggered = 1
            logger.print("SELLTRIGGER")
        # big change down
        elif delta_pct <= -change_threshold_pct:
            # buy
            orders = self.limit_buy(Product.INK, od, best_market_ask, position, position_limit, trader_data)
            threshold_triggered = 1
            logger.print("BUYTRIGGER")

        logger.print("position", position)


        if position > 0:
            logger.print("PREVIOUS BUY PRICE", trader_data['previous_buy_price'])  
            price = trader_data['previous_buy_price'] * (1+ self.params['price_percent_needed'])
            orders.append(Order(Product.INK, price, -position))
            logger.print("CLEAR SELL POSITION", position, price)

        elif position < 0:
            logger.print("PREVIOUS SELL PRICE", trader_data['previous_sell_price'])  

            price = trader_data['previous_sell_price'] * (1 - self.params['price_percent_needed'])
            orders.append(Order(Product.INK, price, -position))
            # orders = self.limit_buy(Product.INK, od, best_market_ask + 5, position, 0)
            logger.print("CLEAR BUY POSITION", -position, price)


        if ink_trigger_hist[-100:] == 1:
            logger.print("FORCE CLEAR")
            if position > 0:
    ...
    def run(self, state: TradingState) -> dict[str, list[Order]]:
        ...
        # set previous buy to last price in history
        trader_data.setdefault('previous_buy_price', None)

        trader_data.setdefault('previous_sell_price', None)


        for product in [Product.INK,]:

            # look through self.own_trades to find the last trade price for this product
            last_trade_price = None
            if product in state.market_trades:
                last_trades = state.market_trades[product]
                # print("last_trades", last_trades)
                
                # loop through the last trades from most recent to least recent
                buy_done = False
                sell_done = False
                for trade in last_trades[::-1]:
                    # print("trade", trade.buyer, trade.seller, trade.price, trade.quantity, trade.timestamp)
                    if state.position.get(product, 0) > 0:
                        trader_data['previous_buy_price'] = (trade.price)
                        buy_done = True
                    else:
                        trader_data['previous_sell_price'] = (trade.price)
                        sell_done = True
                    
                    if buy_done and sell_done:
                        break
            
            # delete the oldest price if there are more than 5
            # if len(trader_data['previous_buy_price']) > 5:
            #     trader_data['previous_buy_price'].pop(0)
            # if len(trader_data['previous_sell_price']) > 5:
            #     trader_data['previous_sell_price'].pop(0)
                order_depth = state.order_depths[product]


            if(trader_data['previous_buy_price'] is None):
                trader_data['previous_buy_price'] = trader_data['ink_price_hist'][0] if trader_data.get('ink_price_hist', None) else None

            if(trader_data['previous_sell_price'] is None):
                trader_data['previous_sell_price'] = trader_data['ink_price_hist'][0] if trader_data.get('ink_price_hist', None) else None
            ...
            if trader_data['previous_buy_price'] is None and trader_data['previous_sell_price'] is None:
                trader_data['previous_buy_price'] = self.mid_price(order_depth)
                trader_data['previous_sell_price'] = self.mid_price(order_depth)
